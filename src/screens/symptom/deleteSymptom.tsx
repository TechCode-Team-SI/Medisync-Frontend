...
export function deleteSymptom() {
  ...
  const DeleteInjury = useMutation({
    mutationKey: [''],
    mutationFn: SymptomHttp.deleteSymptoms,
    onSuccess: () => {
      toast.success('Sintoma Eliminada Correctamente');
      console.log('Eliminado');
      refetch();
    },
    onError: () => {
      toast.error('Sintoma Eliminado Correctamente');
    ...
    <div className='w-full h-full flex flex-col items-center bg-green-400 relative'>
      <Card className='h-full w-full flex flex-col px-8 sm:px-9 lg:px-10 pt-8 sm:pt-9 lg:pt-10 bg-green-600 border-none rounded-none rounded-l-xl'>
        ...
        <Card className='bg-white w-full h-full rounded-b-none overflow-auto scrollbar-edit flex flex-col p-6 pb-0 sm:p-8 sm:pb-0 lg:p-10 lg:pb-0 space-y-5'>
          <MainContentWrapper.Header withBrowser setSearchTerm={setSearchTerm} title='ELIMINAR SINTOMA' />
