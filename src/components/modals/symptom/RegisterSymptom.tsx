...
export function RegisterSymptom({
  ...
  const registreSymptom = useMutation({
    mutationKey: [''],
    mutationFn: SymptomHttp.postSymptoms,
    onSuccess: () => {
      console.log('creado');
      setModalCheckOpen(true);
    },
    onError: () => {
      console.log(registreSymptom.error?.message);
    },
  });

  const EditSymptom = useMutation({
    mutationKey: [''],
    mutationFn: SymptomHttp.patchSymptoms,
    onSuccess: () => {
      console.log('Editado');
      setModalCheckOpen(true);
    },
    onError: () => {
      console.log(EditSymptom.error?.message);
    },
  });

  const onSubmit = (data: InjurySchema) => {
    if (!id) {
      registreSymptom.mutate({
        name: data.name,
            ...
            <form className='space-y-4 ' onSubmit={form.handleSubmit(onSubmit)}>
              ...
              <div className='flex flex-row justify-center p-4'>
                <Button
                  ...
                  type='submit'
                  variant={'btnGreen'}
                  disabled={registreSymptom.isPending || EditSymptom.isPending}
                >
                  {id === undefined ? (
                    registreSymptom.isPending ? (
                      <Spinner />
                    ) : (
                      'Guardar'
                    )
                  ) : EditSymptom.isPending ? (
                    <Spinner />
                  ) : (
                    'Editar'
                  )}
