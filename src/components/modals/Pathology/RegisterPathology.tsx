...
export function RegisterPathology({
  ...
  const registrePathology = useMutation({
    mutationKey: [''],
    mutationFn: PathologyHttp.postPathology,
    onSuccess: () => {
      console.log('creado');
      setModalCheckOpen(true);
    },
    onError: () => {
      console.log(registrePathology.error?.message);
    },
  });

  const EditPathology = useMutation({
    mutationKey: [''],
    mutationFn: PathologyHttp.patchPathology,
    onSuccess: () => {
      console.log('Editado');
      setModalCheckOpen(true);
    },
    onError: () => {
      console.log(EditPathology.error?.message);
    },
  });

  const onSubmit = (data: InjurySchema) => {
    if (!id) {
      registrePathology.mutate({
        name: data.name,
        description: data.description,
      });
    } else {
      EditPathology.mutate({
        id: id,
        name: data.name,
        description: data.description,
      });
            ...
            <form className='space-y-4 ' onSubmit={form.handleSubmit(onSubmit)}>
              ...
              <div className='flex flex-row justify-center p-4'>
                <Button
                  ...
                  type='submit'
                  variant={'btnGreen'}
                  disabled={registrePathology.isPending || EditPathology.isPending}
                >
                  {id === undefined ? (
                    registrePathology.isPending ? (
                      <Spinner />
                    ) : (
                      'Guardar'
                    )
                  ) : EditPathology.isPending ? (
                    <Spinner />
                  ) : (
                    'Editar'
                  )}
