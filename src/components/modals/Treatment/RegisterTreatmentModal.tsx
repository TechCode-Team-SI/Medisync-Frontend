...
export function RegisterTreatmentModal({
  ...
  const form = useForm<TreatmentSchema>({
    ...
    defaultValues: { name: titleTreatment, description: descriptionTreatment },
  });

  const registreTreatment = useMutation({
    mutationKey: [''],
    mutationFn: TreatmentHttp.postTreatment,
    onSuccess: () => {
      console.log('creado');
      setModalCheckOpen(true);
    },
    onError: () => {
      console.log(registreTreatment.error?.message);
    },
  });

  const EditTreatment = useMutation({
    mutationKey: [''],
    mutationFn: TreatmentHttp.patchTreatment,
    onSuccess: () => {
      console.log('Editado');
      setModalCheckOpen(true);
    },
    onError: () => {
      console.log(EditTreatment.error?.message);
    },
  });

  const onSubmit = (data: TreatmentSchema) => {
    if (!id) {
      registreTreatment.mutate({
        name: data.name,
            ...
            <form className='space-y-4 ' onSubmit={form.handleSubmit(onSubmit)}>
              ...
              <div className='flex flex-row justify-center p-4'>
                <Button
                  ...
                  type='submit'
                  variant={'btnGreen'}
                  disabled={registreTreatment.isPending || EditTreatment.isPending}
                >
                  {id === undefined ? (
                    registreTreatment.isPending ? (
                      <Spinner />
                    ) : (
                      'Guardar'
                    )
