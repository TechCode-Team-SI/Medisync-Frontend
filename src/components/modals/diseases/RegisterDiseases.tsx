...
export function RegisterDisease({
  title,
  titleInjury,
  descriptionInjury,
  alert,
  ...
  const registreDisease = useMutation({
    mutationKey: [''],
    mutationFn: DiseaseHttp.postDisease,
    onSuccess: () => {
      console.log('creado');
      setModalCheckOpen(true);
    },
    onError: () => {
      console.log(registreDisease.error?.message);
    },
  });

  const EditDisease = useMutation({
    mutationKey: [''],
    mutationFn: DiseaseHttp.patchDisease,
    onSuccess: () => {
      console.log('Editado');
      setModalCheckOpen(true);
    },
    onError: () => {
      console.log(EditDisease.error?.message);
    },
  });

  const onSubmit = (data: InjurySchema) => {
    if (!id) {
      registreDisease.mutate({
        name: data.name,
            ...
            <form className='space-y-4 ' onSubmit={form.handleSubmit(onSubmit)}>
              ...
              <div className='flex flex-row justify-center p-4'>
                <Button
                  ...
                  type='submit'
                  variant={'btnGreen'}
                  disabled={registreDisease.isPending || EditDisease.isPending}
                >
                  {id === undefined ? (
                    registreDisease.isPending ? (
                      <Spinner />
                    ) : (
                      'Guardar'
                    )
                  ) : EditDisease.isPending ? (
                    <Spinner />
                  ) : (
                    'Editar'
                  )}
